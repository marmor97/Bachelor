...
prepare_file <- function(filename, path = getwd()) {
  ...
  file$Featureset   <- rep(paste0(regmatches(filename, gregexpr("[a-z]*", filename))[[1]][1]),4)
                      ...
                      if ((regmatches(filename, gregexpr("[a-z]*", filename))[[1]][5]) == "sigmoid"){
  file$Kernel       <- rep("Sigmoid", 4)
                      }
  
                      else if ((regmatches(filename, gregexpr("[a-z]*", filename))[[1]][5]) == "rbf"){
  file$Kernel       <- rep("RBF", 4)
                      }
...
plot_performance <- function(data, title){
  data %>% 
    ...
    labs(x = "Kernel",y = "F-score", title = title, fill = "Model")+
    scale_fill_brewer(palette = "Accent") + #Accent is good
