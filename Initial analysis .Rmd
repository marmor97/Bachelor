...
missing_id <- setdiff(u_demo,u_gemap)
...
LASSO IN CARET
```{r}
caret_lambda_grid <- expand.grid(alpha = 1, lambda = lambdas_to_try)

k_folds = uniqueN(train_scaled_clean$ID)
lasso_folds <- caret::groupKFold(group = train_scaled_clean$ID, k = k.folds) 

fold_train <- groupdata2::fold(train_scaled_clean,
                               k = uniqueN(train_scaled_clean$ID),
                               id_col = "ID"
)

control <- caret::trainControl( # k Folds grouped by subject cross validation, repeated 3 times
    method = "LOOCV", 
    number = k_folds, 
    # repeats = 3,
    index = lasso_folds,
...
lambdas_to_try <- 
lasso_caret<- caret::train(x, 
                    y, 
                    method = "glmnet", 
                    tuneGrid = caret_lambda_grid, 
...
train_lasso_features <- train_scaled_clean[,c("ID", "Diagnosis", "Gender"), colnames(train_scaled_clean[train_scaled_clean]) %in% lasso_coef$term] #this makes a new dataframe where there are only the coefficients selected in our lasso feature reduction ##GENDER KOMMER IKKE MED
